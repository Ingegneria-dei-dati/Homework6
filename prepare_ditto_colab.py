...
# Base path su Drive (cartelle già create su Colab)
BASE_DRIVE = "/content/FAIR-DA4ER/data"
...
# Output su Drive
OUT_B1 = os.path.join(BASE_DRIVE, "cars_B1")
OUT_B2 = os.path.join(BASE_DRIVE, "cars_B2")
...
CHUNK_SIZE = 50_000  # dimensione chunk per CSV
LOG_EVERY = 100_000  # log ogni N coppie
...
def build_recs_map():
    """Serializza i record in stringhe Ditto in streaming per non esplodere RAM"""
...
def generate_pairs(blocks, recs, id_to_vin):
    """Generatore memory-safe per coppie dai blocchi"""
    for ids in blocks.values():
        ...
        for a, b in combinations(valid_ids, 2):
            ...
            yield f"{recs[a]}\t{recs[b]}\t{label}"

def write_pairs_from_blocks(blocks, recs, id_to_vin, out_path):
    """Scrive coppie da blocchi su file, memory-safe, con log di avanzamento"""
    count = 0
    start_time = time.time()

    with open(out_path, "w", encoding="utf-8") as f:
        for pair in generate_pairs(blocks, recs, id_to_vin):
            f.write(pair + "\n")
            count += 1
...
def main():
    ...
    for strat, out_dir in [("B1", OUT_B1), ("B2", OUT_B2)]:
        ...
        print(f"  ✅ train.txt scritto in {train_path}")

        # VAL / TEST dai blocchi
