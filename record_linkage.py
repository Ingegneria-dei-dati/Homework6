...
def run_complete_evaluation_suite():
    ...
    # --- NORMALIZZAZIONE MINIMA (Per evitare errori e uniformare i dati) ---
    for col in ['make', 'model']:
        # Uso di .str accessor per evitare AttributeError
        df[col] = df[col].fillna('').astype(str).str.lower().str.strip()
    
    # Uniformiamo l'anno (es. 2019.0 -> 2019) per il compare.exact
    df['year'] = df['year'].fillna('0').astype(str).str.replace('.0', '', regex=False).str.strip()
    # -----------------------------------------------------------------------
    ...
    # Pesi decisi in base alla conoscenza del dominio (come da punto 4.H della relazione)
    weights = np.array([2.0, 4.0, 2.0, 1.0, 1.0])
    weight_sum = weights.sum()
    ...
    print("Inizio analisi Record Linkage Manuale (Baseline)...")
    ...
    for split in splits:
        for strat in strategies:
            ...
            # --- FASE TRAINING (Tempo di definizione pesi manuale) ---
            start_train = time.time()
            _ = np.array([2.0, 4.0, 2.0, 1.0, 1.0]) 
            end_train = time.time()
            training_time = end_train - start_train
            ...
            df_index_set = set(df.index)
            for block_ids in blocks.values():
                # Soglia di sicurezza 200 per stabilità RAM
                if 1 < len(block_ids) < 200: 
                    valid_ids = [str(idx) for idx in block_ids if str(idx) in df_index_set]
                    if len(valid_ids) >= 2:
                        a = np.array(valid_ids)
            ...
            compare.string('model', 'model', method='jarowinkler', threshold=0.85)
            compare.exact('year', 'year')
            # Comparazione numerica senza offset (massima precisione richiesta per baseline)
            compare.numeric('price', 'price', method='lin', offset=0, scale=0.1) 
            compare.numeric('mileage', 'mileage', method='lin', offset=0, scale=0.1)
            ...
            # Valutazione tramite Ground Truth (VIN)
            v1 = np.array([id_to_vin.get(str(p[0])) for p in all_pairs])
            v2 = np.array([id_to_vin.get(str(p[1])) for p in all_pairs])
            y_true = ((v1 == v2) & (v1 != None)).astype(int)
    ...
    print("\n" + "="*85)
    print(" 1. STUDIO DELLA SENSIBILITÀ (TEST SET) ".center(85, "="))
    print("="*85)
    print(sensitivity_df[sensitivity_df['Split'] == 'test'].to_string(index=False))

    print("\n" + "="*105)
    print(f" 2. REPORT GLOBALE E TEMPI (SOGLIA FISSA A {SOGLIA_SCELTA}) ".center(105, "="))
    print("="*105)
    print(global_df.sort_values(by=['Split', 'Blocking']).to_string(index=False))

    sensitivity_df.to_csv(os.path.join(results_dir, "manual_sensitivity_analysis.csv"), index=False)
    global_df.to_csv(os.path.join(results_dir, "manual_evaluation_report.csv"), index=False)
